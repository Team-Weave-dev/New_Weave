...
npm run test:watch       # Jest Watch 모드 (개발 중 자동 재실행)
npm run test:coverage    # 테스트 커버리지 리포트 생성
npx playwright test      # E2E 테스트 실행 (크로스브라우저)
...
- **AI 통합**: 다중 AI 공급자 지원 (@google/generative-ai, langchain)
- **상태 관리**: Zustand 스토어
...
├── dashboard/page.tsx         # 커스터마이징 가능한 대시보드
├── projects/                  # 프로젝트 관리
│   ├── page.tsx              # 프로젝트 목록 (마스터-디테일)
│   ├── [projectId]/page.tsx  # 프로젝트 상세
│   └── settings/page.tsx     # 프로젝트 설정
├── ai-assistant/             # AI 업무비서
│   ├── page.tsx             # AI 메인
│   ├── chat/page.tsx        # AI 채팅
│   ├── consult/page.tsx     # AI 상담
│   ├── extract/page.tsx     # 데이터 추출
│   ├── generate/page.tsx    # 문서 생성
│   └── file-process/page.tsx # 파일 처리
├── reminders/page.tsx        # 리마인더 대시보드
├── business-lookup/page.tsx  # 사업자 조회
...
│   ├── PageContainer.tsx    # 페이지 컨테이너 (3가지 타입)
│   └── HomeLayout.tsx       # 홈 페이지 레이아웃
├── navigation/               # 네비게이션 컴포넌트
│   └── MainNavigation.tsx   # 메인 네비게이션
├── ai-assistant/            # AI 관련 특화 컴포넌트
│   ├── WeaveAssistant.tsx   # 메인 AI 어시스턴트
│   ├── DocumentGenerator.tsx # 문서 생성기
│   └── [AI 기능별 컴포넌트들]/
├── dashboard/               # 대시보드 전용 컴포넌트
├── projects/               # 프로젝트 관련 컴포넌트
├── reminders/              # 리마인더 관련 컴포넌트
└── [기능별 컴포넌트들]/
```
...
│   ├── chatService.ts     # AI 채팅 서비스
│   └── supabase/          # Supabase 관련 서비스들
│       ├── projects.service.ts
│       ├── clients.service.ts
│       ├── invoices.service.ts
│       └── [기타 서비스들]/
├── hooks/                 # 커스텀 훅
│   ├── useProjectMasterDetail.ts # 프로젝트 마스터-디테일 훅
│   ├── useProjectTable.ts        # 프로젝트 테이블 관리 훅
│   └── [기타 훅들]/
├── types/                 # TypeScript 타입 정의
├── stores/               # Zustand 스토어
├── auth/                 # 인증 관련
├── dashboard/           # 대시보드 관련 유틸리티
├── rag/                 # RAG 파이프라인
└── utils/               # 유틸리티 함수들
```
...
- `next.config.js`: Next.js 최적화 설정 (이미지, 번들링, 캐시 등)
- `tailwind.config.ts`: Weave 디자인 시스템 색상 및 스타일 정의
- `jest.config.js`: Jest 테스트 설정 (커버리지 80% 목표)
- `playwright.config.ts`: E2E 테스트 설정 (크로스브라우저 지원)
